...
Generated by `bmad-code-review` on 2026-05-30. Three parallel review layers ran against impl commit `680da99`: Blind Hunter (diff-only, no project context), Edge Case Hunter (diff + project read), Acceptance Auditor (diff + spec + context anchors). Raw counts: 22 + 23 + 10 = 55 findings. After dedupe and triage: 5 decision-needed, 9 patch, 6 deferred, 15 dismissed as noise.

#### Decision-needed (5, all resolved 2026-05-30 in code-review session)

- [x] [Review][Decision] **D1 — Image and dependency pinning policy** → **PIN ALL THREE**: digest-pin `python:3.13-slim`, version-floor `'uv>=0.4,<1'`, pin `uvx semvertag@v1`. Converted to **Patch P10**.
- [x] [Review][Decision] **D2 — Quick Start concurrency/safety defaults** → **ADD `resource_group: semvertag` to component-body defaults**. Component-level protection so consumers get serialization even if they ignore the docs. Converted to **Patch P11**.
- [x] [Review][Decision] **D3 — OQ2-OQ7 resolution** → **OQ4: create `docs/strategies/*.md` stubs now + link from `gitlab.md` only** (preserves AC11 for `github.md`; backfill deferred). **OQ5: confirm shipped position** (`SEMVERTAG_GITLAB__ENDPOINT` stays as CI variable, not a component input). **OQ2, OQ3, OQ6, OQ7: defer all four flat** to `deferred-work.md`. OQ4 stub-creation converted to **Patch P12**.
- [x] [Review][Decision] **D4 — Architecture/epics amendment for OQ1** → **ONE-LINE AMENDMENT NOW** in this story; `architecture.md:1067-1069` and `epics.md:779` get correction notes pointing to `templates/semvertag.yml`. Converted to **Patch P13**.
- [x] [Review][Decision] **D5 — Negative-test fixture for CI gate** → **ADD NOW with explicit Constraint 7 exception**. `tests/test_ci_descriptor_gate.py` with one positive fixture + four negative fixtures (drop `options`, drop `default`, drop `---` separator, wrong `${{ }}` substitution syntax). Converted to **Patch P14**.

#### Patch (9)

- [x] [Review][Patch] **AC10 description drift — VIOLATED** — descriptor's `description` (`'Bump strategy. One of: branch-prefix (default), conventional-commits.'`) and docs Inputs-table description (`Bump strategy. One of \`branch-prefix\` or \`conventional-commits\`.`) differ in three places: colon dropped, comma → `or`, `(default)` annotation dropped. AC10 mandates byte-equal. Canonical source per AC10 is the descriptor → update docs. [`docs/providers/gitlab.md:52`, `templates/semvertag.yml:4`]
- [x] [Review][Patch] **Replace `assert` with explicit `raise SystemExit` in CI descriptor gate** — `assert` is stripped under `python -O`; a future toolchain change silently passes broken descriptors. Also improves diagnostics by adding field-name context to each error message. [`.github/workflows/ci.yml:46-66`]
- [x] [Review][Patch] **Use `with open(..., encoding='utf-8') as f:` in CI gate** — current `open('templates/semvertag.yml')` is implicit-encoding + leaked file handle. Pre-empts BOM / CRLF / locale issues on hardened or Windows runners; produces clearer trace on file errors. Combine with the `assert`-removal patch. [`.github/workflows/ci.yml:51`]
- [x] [Review][Patch] **`mkdocs.yml` nav order is reverse-alphabetical** — `GitLab CI` (L) precedes `GitHub Actions` (H); true alphabetical order is the reverse. Story Change Log entry 4 and Task 3.1 both claim "alphabetically before GitHub Actions" — claim is false. Swap to true alphabetical (GitHub Actions first, GitLab CI second) OR amend the story note to acknowledge a non-alphabetical convention. [`mkdocs.yml:7-9`]
- [x] [Review][Patch] **`<org>` placeholder warning missing from second snippet and Inputs-link** — the warning paragraph at `docs/providers/gitlab.md:36-38` only follows the first include snippet. The conventional-commits variant snippet (`~line 113-118`) and the descriptor source link at `~line 53` both embed `<org>` with no adjacent guard. Add identical callout (or single source-of-truth snippet) after every `<org>` occurrence. [`docs/providers/gitlab.md:53`, `docs/providers/gitlab.md:113-118`]
- [x] [Review][Patch] **Change Log entry 3 names wrong source file** — claim is "verified against `semvertag/providers/gitlab.py` to avoid 4-3a's 'fictional error strings' code-review finding". Verified: `Project id missing.` actually lives at `semvertag/ioc.py:36`, not `providers/gitlab.py`. The other strings DO live in `providers/gitlab.py:300-306`. Strings are real but the verification claim misleads future maintainers. Update Change Log entry 3 to reference both `ioc.py` and `providers/gitlab.py`. [`_bmad/4-3b-gitlab-ci-catalog-component.md` Change Log entry 3]
- [x] [Review][Patch] **Token-scope masking note in docs** — `SEMVERTAG_TOKEN` set in CI variables silently masks `CI_JOB_TOKEN` because of the pydantic AliasChoices order at `semvertag/_settings.py:15-20`. A common footgun when migrating from PAT → job-token; current docs (`docs/providers/gitlab.md:60-66`) say "the first set value wins" but never name the masking risk. Add a one-line explicit note in the Token-scope section. [`docs/providers/gitlab.md:60-66`]
- [x] [Review][Patch] **`SEMVERTAG_GITLAB__ENDPOINT` footgun docs** — no warning about three common copy-paste mistakes: trailing slash (handled by code's `rstrip('/')`, but undocumented), missing scheme (fails with httpx `ConnectError`), or appending `/api/v4` (double-prefixed URLs → 404). Also mention the single-underscore typo (`SEMVERTAG_GITLAB_ENDPOINT`) since the docs already call out "note the double underscore". Add one or two lines to the self-hosted callout. [`docs/providers/gitlab.md:78-81`, `docs/providers/gitlab.md:136-140`]
- [x] [Review][Patch] **`set(inputs) == {'strategy'}` gate is brittle on `None`/non-dict input** — if a future descriptor edit has `inputs:` with nothing under it, `spec['spec']['inputs']` is `None` and `set(None)` raises `TypeError` with no key-name context. Add `isinstance(inputs, dict)` guard with a diagnostic message before the `set(...)` call (folds into the `assert`-removal patch). [`.github/workflows/ci.yml:55`]
- [x] [Review][Patch] **P10 (from D1): pin descriptor image and dependencies** — `templates/semvertag.yml`: digest-pin `python:3.13-slim` (resolve the current digest with `docker manifest inspect python:3.13-slim` or via the registry's API); pin `pip install --quiet --no-cache-dir 'uv>=0.4,<1'` (version floor with major-version cap); change `uvx semvertag` to `uvx 'semvertag@v1'` so the Catalog `@v1` consumer pin propagates to the CLI version. Update the CI gate (P2/P3 area) to assert that `before_script[0]` contains a version specifier and `script[0]` contains `@v` to prevent future un-pinning. [`templates/semvertag.yml:1-16`, `.github/workflows/ci.yml:46-66`]
- [x] [Review][Patch] **P11 (from D2): add `resource_group: semvertag` to component-body defaults** — insert `resource_group: semvertag` as a top-level key on the `semvertag:` job in the component body. GitLab serializes jobs sharing a `resource_group` across pipelines; this prevents concurrent `create_tag` POSTs from racing into 400 "already exists" errors. Also update the AC5 gate to assert `'resource_group' in job` (folds into P2/P3 patch). Update `docs/providers/gitlab.md` Inputs / Quick Start section to mention the default and how to override it for projects that want concurrent tag pushes. [`templates/semvertag.yml:9-16`, `.github/workflows/ci.yml:46-66`, `docs/providers/gitlab.md`]
- [x] [Review][Patch] **P12 (from D3a-revised): create `docs/strategies/{branch-prefix,conventional-commits}.md` stubs and link from `gitlab.md`** — both stubs: H1 (`# Branch-prefix strategy` / `# Conventional Commits strategy`) + 1-paragraph intro pulled from the existing prose in `providers/gitlab.md` (and `providers/github.md` for parity baseline) + "See provider docs for the consumer integration: [GitLab CI](../providers/gitlab.md) / [GitHub Actions](../providers/github.md)" backlinks. Add nav entries to `mkdocs.yml` under a new `Strategies:` parent (alphabetized: `Branch prefix`, `Conventional commits`). Add inline `(see [Branch-prefix strategy](../strategies/branch-prefix.md))` links in `providers/gitlab.md`'s "Branch-prefix vs conventional-commits" section. AC11 prohibits touching `providers/github.md` in this story — the symmetric backfill is captured as a deferred-work item. Verify `mkdocs build --strict` stays green. [`docs/strategies/branch-prefix.md` (NEW), `docs/strategies/conventional-commits.md` (NEW), `mkdocs.yml`, `docs/providers/gitlab.md`]
- [x] [Review][Patch] **P13 (from D4): amend `architecture.md:1067-1069` + `epics.md:779`** — both currently prescribe `.gitlab/catalog/component.yml`. Add a one-line correction note (or rewrite the descriptor-path sentence in place) pointing to the actual ship path `templates/semvertag.yml`, citing Story 4.3b's OQ1 resolution + GitLab Catalog ingestion behavior. Preserve surrounding prose; minimal-diff edit. [`_bmad/architecture.md:1067-1069`, `_bmad/epics.md:779`]
- [x] [Review][Patch] **P14 (from D5): add `tests/test_ci_descriptor_gate.py`** — five test cases: (1) positive — current shipped descriptor passes the inline gate logic (assert `validate_descriptor('templates/semvertag.yml') is None` or similar); (2) negative — `options` key absent → raises with field-name in message; (3) negative — `default` key absent → raises; (4) negative — `---` separator removed (one-document layout) → raises with "expected 2 YAML docs" message; (5) negative — substitution syntax changed to `${{ inputs.strategy }}` (GitHub Actions form, the AP1 footgun) → raises. Implementation choice: either (a) factor the gate logic into a small `_validate_descriptor.py` module that both the CI step and the tests import; (b) keep gate inline in ci.yml + have the tests subprocess-shell-out to run the heredoc. Approach (a) is cleaner and is the recommended path. **Requires explicit Constraint 7 exception**: document the exception in Change Log entry for this code-review pass. [`tests/test_ci_descriptor_gate.py` (NEW), `semvertag/_validate_descriptor.py` (NEW, if approach (a)), `.github/workflows/ci.yml:46-66`]

#### Deferred (6)

- [x] [Review][Defer] **`semvertag-0` build artifact — package version literally `0`** [`pyproject.toml` (not in 4-3b diff)] — deferred, pre-existing. `uv build` produces `dist/semvertag-0.tar.gz`. Indicates a missing/placeholder `version =` or broken dynamic-version config. Not introduced by 4-3b; visible in this story's Debug Log only because gates were captured. Owner: future release-prep story (Epic 4 trust-surface or publish-flow).
- [x] [Review][Defer] **`mkdocs.yml` `repo_url` and `extra.social` link still contain literal `<org>`** [`mkdocs.yml` (not in 4-3b diff)] — deferred, pre-existing. Renders into the published docs site as a dead link. Same `<org>` placeholder hazard as the descriptor include; tracked alongside the existing `deferred-work.md §1-1` line 12 item. Source: Edge Hunter #21.
- [x] [Review][Defer] **`templates/` glob enforcement / single-component policy** [`.github/workflows/ci.yml:51`] — deferred, no current breach. Gate hardcodes `templates/semvertag.yml`; a future second component under `templates/` would silently bypass. No second component planned; revisit if/when one lands. Source: Edge Hunter #19.
- [x] [Review][Defer] **`options:` hardcoded to two strategies in CI gate** [`.github/workflows/ci.yml:60`] — deferred, future-proof concern. `assert sorted(s.get('options', [])) == ['branch-prefix', 'conventional-commits']` is an exact match. When a third strategy lands (post-Epic-4), this gate fails CI and forces the developer to update it — which is acceptable but worth a heads-up in the strategy-author docs. Source: Edge Hunter #15.
- [x] [Review][Defer] **End-to-end smoke job for the descriptor runtime** [`.github/workflows/ci.yml` (absence)] — deferred, scope. No CI job runs `docker run python:3.13-slim sh -c "pip install uv && uvx semvertag --version"` to catch upstream image / `uv` / PyPI regressions. The descriptor is currently a publish-and-pray surface. Bigger than 4-3b; sized as its own story. Source: Edge Hunter #6, #22.
- [x] [Review][Defer] **`mkdocs.yml` Providers section has no index page** [`mkdocs.yml:7-9`] — deferred, editorial-hierarchy choice. Clicking "Providers" lands on whichever child mkdocs auto-selects (currently GitLab CI, post-swap GitHub Actions). Adding a `Providers: providers/index.md` requires a new `docs/providers/index.md`. Out of 4-3b scope; revisit in Story 4-4 (mkdocs site content). Source: Blind Hunter #18.

#### Dismissed (15, summarized)

These were raised by one or more layers and reviewed but judged non-actionable: heredoc shell-quoting safety (correctly `<<'PY'`), missing `set -euo pipefail` (Actions default `bash -e` suffices), smoke-check key-order paraphrased in dev notes (cosmetic), token alias chain "unverifiable from diff" (verified against `_settings.py:15-20`), troubleshooting error strings "unverifiable from diff" (verified against `ioc.py:36` + `providers/gitlab.py:300-306`), AC14 self-checked pending CI run (CI-run-dependent by design), pre-publish docs hazard (acknowledged in AC9), `type: string` + `options:` paradigm (correct per AC2), missing `semvertag doctor` CI invocation example (nit, doctor docs lives elsewhere), `import sys` dropped from AC5 heredoc (harmless — arguably cleaner), AC2-prose vs Constraint 9 contradiction (spec-text issue, not code), `<org>` in descriptor link (folded into Patch P5 + Defer W2), `stage:` not declared in component (GitLab Catalog convention; consumer redeclares to add `stage:`/`rules:`), description prose duplicates `default:` field (superseded by Patch P1), `yaml.safe_load_all` lazy-trace diagnostics (subsumed by Patch P3).
...
3. **Task 2:** Authored `docs/providers/gitlab.md`. Followed `github.md`'s six-section convention. Notable deviations from `github.md`:
   ...
   - Troubleshooting bullets describe ACTUAL CLI/API error symptoms — verified against `semvertag/providers/gitlab.py:300-306` (the 403 / 404 / scope strings emitted by `_translate_status`) AND `semvertag/ioc.py:36` (the `Project id missing. Set CI_PROJECT_ID or pass --project-id.` string). The dev's original Change Log entry incorrectly attributed all troubleshooting strings to `providers/gitlab.py` alone; that attribution was corrected during code review (2026-05-30, Patch P6) after the Edge Hunter caught `Project id missing.` was not in `providers/gitlab.py`. The strings are real; the verification claim is now accurate.
