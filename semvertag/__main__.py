...
def _doctor_command(  # noqa: PLR0913
    ...
    json_flag: typing.Annotated[
        bool,
        typer.Option("--json", help="Emit a single-line JSON envelope instead of human-readable output."),
    ] = False,
) -> None:
    """
    Run four pre-flight checks against the configured provider: token, scopes, project_access, protected_tags.

    Emits a configuration table + checks table to stdout, or a single-line JSON
    envelope when --json is set. Exits 0 when all pass; 3 (auth) / 2 (config) /
    4 (provider) / 1 (generic) on failure per FR37.
    """
